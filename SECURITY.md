...
- **Admin and guardian keys are immutable.** There is no `set_admin` or `set_guardian` instruction. If either key is lost or compromised, recovery requires redeploying the program. Verify the admin pubkey on-chain before relying on the program.
